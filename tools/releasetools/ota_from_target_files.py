...
""" % bcb_dev)
  ...
  if "selinux_fc" in OPTIONS.info_dict:
    ...
    version = GetBuildProp("ro.lite.version", OPTIONS.info_dict)

  
  if GetBuildProp("ro.product.model", OPTIONS.info_dict) is not None:
    model = GetBuildProp("ro.product.model", OPTIONS.info_dict)
    script.Print("***********************************************");
    script.Print("           LiteOS for %s"%(model));
    script.Print("     LiteOS Version: %s"%(version));
...
""" % bcb_dev)
  ...
  common.ZipWriteStr(output_zip, "META-INF/org/aospgs/releasekey",
